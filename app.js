...
app.post("/HackUtsav", async (req, res) => {
    try {
        ...
        // Validate required fields
        if (!email1) {
            return res.status(400).render("new.ejs", { error: "Email1 is required" });
        }

        if (members == 2 && (!email2 || !contact2)) {
            return res.status(400).render("new.ejs", { error: "Email2 and Contact2 are required for two members" });
        }

        if (members == 3 && (!email3 || !contact3)) {
            return res.status(400).render("new.ejs", { error: "Email3 and Contact3 are required for three members" });
        }

        if (!interest) {
            return res.status(400).render("new.ejs", { error: "Area of Interest is required." });
        }
        ...
        if (existingRegisterByEmail1) {
            return res.status(400).render("new.ejs", { error: "Email1 is already registered." });
        }

        if (existingRegisterByEmail2) {
            return res.status(400).render("new.ejs", { error: "Email2 is already registered." });
        }

        if (existingRegisterByContact1) {
            return res.status(400).render("new.ejs", { error: "Contact number of member 1 is already registered." });
        }

        if (existingRegisterByContact2) {
            return res.status(400).render("new.ejs", { error: "Contact number of member 2 is already registered." });
        }

        if (existingRegisterByPaymentId) {
            return res.status(400).render("new.ejs", { error: "UPI Payment ID is already used." });
        }
        ...
        // Render success page with a flash message
        res.status(200).render("success.ejs");
    } catch (error) {
        console.error("Error saving registration:", error);
        res.status(500).render("new.ejs", { error: "Error registering. Please try again." });
    }
});
